#!/usr/bin/env bash
# resume.sh — Resume a paused autoresearch fleet
#
...
die()     { echo -e "${RED}[resume]${NC} $*" >&2; exit 1; }
...
if [[ -f "${FLEET_JSON}" ]]; then
  ...
  jq --arg ts "${local_ts}" '.status = "running" | .resumed_at = $ts' "${FLEET_JSON}" > "${tmp}"
  mv "${tmp}" "${FLEET_JSON}"
fi

success "Fleet '${BOLD}${FLEET_NAME}${NC}${GREEN}' resumed."
info "Orchestrator will continue on next poll."
