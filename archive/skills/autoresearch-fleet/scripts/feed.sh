...
build_jq_filter() {
  ...
  if [[ -n "$type_filter" ]]; then
    # Match event, type, subtype, or Pi message.role field
    conditions+=(
      "(.event == \"$type_filter\" or .type == \"$type_filter\" or .subtype == \"$type_filter\" or (.type == \"message\" and .message.role == \"$type_filter\"))"
    )
  fi
...
for line in sys.stdin:
    ...
    # Pi message events: derive event from message.role
    msg = ev.get("message") or {}
    msg_role = msg.get("role", "")
    if event == "?" and msg_role:
        event = f"message/{msg_role}"
    ...
    if "fail" in event.lower() or "error" in event.lower() or error:
        event_col = RED
         ...
         (subtype and "success" in subtype.lower()) or \
         (msg_role == "assistant" and msg.get("stopReason") == "stop"):
        event_col = GREEN
    elif "stuck" in event.lower() or "blocked" in event.lower() or \
         "retry" in event.lower() or "rate_limit" in event.lower():
        event_col = YELLOW
    elif "tool" in event.lower() or msg_role == "toolResult":
        event_col = MAGENTA
    else:
        event_col = WHITE
    ...
    skip_keys = {"ts","timestamp","agent_id","worker_id","event","type","subtype","tool","error","message"}
    extra_parts = []
    ...
    # Pi: show usage summary from message.usage
    if msg and "usage" in msg:
        u = msg["usage"]
        cost_val = u.get("cost", {}).get("total", 0)
        tokens = u.get("totalTokens", 0)
        extra_parts.append(f"{GRAY}tokens{RESET}={CYAN}{tokens}{RESET}")
        if cost_val and float(cost_val) > 0:
            extra_parts.append(f"{GRAY}cost{RESET}={CYAN}${float(cost_val):.4f}{RESET}")
